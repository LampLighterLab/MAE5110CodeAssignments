...
def pendulum_simulation(integrator, timestep, sim_time, initial_state, params):
    ...
    for step, t in enumerate(time_traj[:-1]):
        state_traj[:, step + 1] = integrator(
            state_traj[:, step], timestep, model.dynamics, t, params)
    ...
    # If damping is present, we will consider all timesteps to be unstable.
...
for timestep in timesteps:
    state_traj, time_traj, stable = pendulum_simulation(explicit_euler, timestep, sim_time, initial_state, params)
    if stable:
        euler_stable_timesteps.append(timestep)
    state_traj, time_traj, stable = pendulum_simulation(rk4, timestep, sim_time, initial_state, params)
    if stable:
...
euler_state_traj, euler_time_traj, _ = pendulum_simulation(explicit_euler, euler_largest_stable_timestep, sim_time, initial_state, params)
rk4_state_traj, rk4_time_traj, _ = pendulum_simulation(rk4, rk4_largest_stable_timestep, sim_time, initial_state, params)
...
t_euler_same = timeit.timeit(lambda: pendulum_simulation(explicit_euler, 1e-5, sim_time, initial_state, params), number=10)
t_rk4_same = timeit.timeit(lambda: pendulum_simulation(rk4, 1e-5, sim_time, initial_state, params), number=10)

# Largest timestep that stayed numerically accurate for each integrator
t_euler_max = timeit.timeit(lambda: pendulum_simulation(explicit_euler, euler_largest_stable_timestep, sim_time, initial_state, params), number=10)
t_rk4_max = timeit.timeit(lambda: pendulum_simulation(rk4, rk4_largest_stable_timestep, sim_time, initial_state, params), number=10)
...
plt.xlabel("Time (s)")
plt.ylabel("Energy (J)")
plt.title(f"Explict Euler Pendulum Energy (dt = {euler_largest_stable_timestep:.5f})")
plt.legend()
plt.tight_layout()
plt.show()
...
plt.title(f"Explict Euler Pendulum Phase Portrait (dt = {euler_largest_stable_timestep:.5f})")
...
plt.xlabel("Time (s)")
plt.ylabel("Energy (J)")
plt.title(f"Runge-Kutta Pendulum Energy (dt = {rk4_largest_stable_timestep:.5f})")
plt.legend()
plt.tight_layout()
plt.show()
...
plt.xlabel("Angle (rad)")
plt.ylabel("Angular Velocity (rad/s)")
plt.title(f"Runge-Kutta Pendulum Phase Portrait (dt = {rk4_largest_stable_timestep:.5f})")
plt.axhline(0, color='k', linestyle='-', linewidth=1)  # Vertical line at x=0 to better see any deviations
plt.tight_layout()
plt.show()