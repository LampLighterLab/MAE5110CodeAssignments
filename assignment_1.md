...
In panel (a), the horizontal limits are exactly the admissible stance-angle interval, $[\gamma-\alpha,\gamma+\alpha]=[-17.5^\circ,27.5^\circ]$; automatic tick labels need not include the endpoints. Equilibrium markers and the unused unresolved legend entry are omitted for clarity; the underlying classifications are unchanged. Basin boundaries are the interfaces between the background colors.
...
The outputs are [the basin figure](figures/state_space_basins.png) and [the grid and classification data](figures/state_space_basins.npz). Subsequent parameter studies should compare how these basins and the walking cycle change with slope and number of spokes.
...
Outputs are [the return-map figure](figures/poincare_map.png), [the sampled map data](figures/poincare_map.npz), and [the numerical measurements](figures/poincare_map.json). The next parameter study can compare how slope and spoke count affect the admissible walking domain, the basins of attraction, and this local convergence multiplier.
...
The results are saved as [basin panels](figures/parameter_basins.png), [parameter trends](figures/parameter_trends.png), [grid classifications](figures/parameter_study.npz), and [measurements and refinement results](figures/parameter_study.json). The earlier interactive scripts and their settings are left unchanged.
